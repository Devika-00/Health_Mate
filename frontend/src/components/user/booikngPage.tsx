...
const AppointmentBookingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchTimeSlots = async () => {
      try {
        const response = await axiosJWT.get(`${USER_API}/time-slots/${id}`);
  ...
  const handlePackageSelection = (packageName: string) => {
    ...
    setIsPackageSelected(true); // Update state to indicate package is selected
  };

  const handleTimeSlotSelection = (timeSlot: string) => {
    setSelectedTimeSlot(timeSlot);
    setIsTimeSlotSelected(true); // Update state to indicate time slot is selected
  };
      ...
      {doctor && (
        <div>
          ...
          <div className="flex justify-between mb-4">
            <button onClick={handleBookAppointment} className="bg-blue-900 text-white py-2 px-4 rounded-lg">
              Book an Appointment
            </button>
          </div>
